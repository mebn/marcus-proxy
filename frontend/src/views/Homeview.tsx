...
export default function Homeview() {
  ...
  const [requestProjectIDs, setRequestProjectIDs] = useState<Record<number, string>>({});
  const [storageReady, setStorageReady] = useState(false);
  const [isDark, setIsDark] = useState(true);
  const [isCapturing, setIsCapturing] = useState(true);
  const isCapturingRef = useRef(isCapturing);
  const activeProjectIDRef = useRef(activeProjectID);
  const proxyURL = useMemo(() => status.lanUrls[0] ?? status.address, [status.address, status.lanUrls]);
  ...
  const projectEntries = useMemo(
    () => status.recent.filter((entry) => projectIDFor(entry.id, requestProjectIDs) === activeProjectID),
    [activeProjectID, requestProjectIDs, status.recent],
  );
  const projectCounts = useMemo(() => countProjects(projects, requestProjectIDs, status.recent), [
    projects,
    requestProjectIDs,
    status.recent,
  ]);
  ...
  const hostStats = useMemo(() => buildHostStats(projectEntries), [projectEntries]);
  ...
  const contentTypeOptions = useMemo(
    () => sortedUnique([...contentTypeFilters, ...projectEntries.flatMap(getContentTypes)]),
    [contentTypeFilters, projectEntries],
  );
  const visibleEntries = useMemo(
    () => filterAndSortEntries(projectEntries, filter, hostFilter, methodFilters, contentTypeFilters, sort),
    [contentTypeFilters, filter, hostFilter, methodFilters, projectEntries, sort],
  );
  ...
  function saveNoProjectSession() {
    ...
    setRequestProjectIDs((current) => assignDefaultRequests(current, status.recent, project.id));
  ...
  function confirmDeleteProject() {
    ...
    setStatus((current) => ({
      ...current,
      recent: current.recent.filter((entry) => projectIDFor(entry.id, requestProjectIDs) !== projectID),
    }));
    setRequestProjectIDs((current) => removeProjectRequests(current, projectID));
    setPinnedIDs((current) => current.filter((id) => projectIDFor(id, requestProjectIDs) !== projectID));
    if (projectID !== defaultProjectID) setProjects((current) => current.filter((project) => project.id !== projectID));
  ...
  function startSidePanelResize(event: React.PointerEvent<HTMLElement>, side: "left" | "right") {
  ...
  useEffect(() => {
    ...
    return EventsOn("traffic:new", (entry: TrafficEntry) => {
      ...
      setRequestProjectIDs((current) => ({ ...current, [entry.id]: activeProjectIDRef.current }));
      setStatus((current) => ({
        ...current,
        recent: [entry, ...(current.recent ?? []).filter((item) => item.id !== entry.id)],
      }));
    });
  }, []);
  useEffect(() => {
  ...
  useEffect(() => EventsOn("theme:dark-mode", (enabled: boolean) => setIsDark(Boolean(enabled))), []);
  useEffect(() => EventsOn("certificate:error", (message: string) => setError(message)), []);
  useEffect(() => EventsOn("certificate:regenerated", onCertificateRegenerated), []);
  useEffect(() => {
  ...
  function applySavedState(savedState: SavedAppState) {
    ...
    setContentTypeFilters((ui.contentTypeFilters ?? []).map(normalizeContentType));
    setActiveProjectID(activeID);
    activeProjectIDRef.current = activeID;
    if (ui.sort?.key && ui.sort?.direction) {
      setSort({ key: ui.sort.key as SortKey, direction: ui.sort.direction as SortDirection });
    }
  }
  function saveState() {
    void SaveAppState({
      ...
      <section className="flex min-h-0 w-full flex-1 bg-card">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="flex min-h-0 flex-1 flex-col bg-card">
            <TrafficTable
              ...
              sort={sort}
              onOpen={openEntry}
              onPin={(entry) => setPinnedIDs((current) => (current.includes(entry.id) ? current : [entry.id, ...current]))}
              onSort={sortBy}
            />
            {detailsOpen ? (
              <RequestDetails
        ...
        {rightPanelOpen ? (
          <RequestSidebar
            ...
            onUnpin={(id) => setPinnedIDs((current) => current.filter((entryID) => entryID !== id))}
          />
        ) : null}
      </section>
      <ConfirmDialog
...
function countProjects(projects: Project[], requestProjectIDs: Record<number, string>, entries: TrafficEntry[]) {
  const counts = new Map<string, number>(projects.map((project) => [project.id, 0]));
  for (const entry of entries) {
...
function buildHostStats(entries: TrafficEntry[]): HostStat[] {
  ...
  return Array.from(counts, ([host, count]) => ({ host, count })).sort((left, right) =>
...
function isVisibleEntry(
  ...
  if (methodFilters.length > 0 && !methodFilters.includes(getMethodType(entry))) return false;
  if (contentTypeFilters.length > 0 && !getContentTypes(entry).some((type) => contentTypeFilters.includes(type))) return false;
...
function assignDefaultRequests(current: Record<number, string>, entries: TrafficEntry[], projectID: string) {
  const next = { ...current };
  for (const entry of entries) {
    if (projectIDFor(entry.id, next) === defaultProjectID) next[entry.id] = projectID;
  }
  return next;
}
function removeProjectRequests(current: Record<number, string>, projectID: string) {
  const next = { ...current };
...
function projectIDFor(entryID: number, requestProjectIDs: Record<number, string>) {
...
function numberKeyMap(values: Record<string, string>) {
  return Object.fromEntries(Object.entries(values).map(([id, sessionID]) => [Number(id), sessionID]));
}
function stringKeyMap(values: Record<number, string>) {
  return Object.fromEntries(Object.entries(values).map(([id, sessionID]) => [String(id), sessionID]));
}
...
function shouldBlockRubberBand(event: WheelEvent) {
  ...
  if (Math.abs(event.deltaY) >= Math.abs(event.deltaX)) return !canScrollY || blocksY;
